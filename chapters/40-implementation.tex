...
図\ref{fig:vgml_katateisu}に示す型・定数定義リストと、図\ref{fig:instance_list}に示すインスタンス変数定義リスト、
    ...
    \item インスタンス変数定義リストの行の数だけ、以下の処理を繰り返す。
        \begin{enumerate}
            ...
            \label{instance_loop}
                ...
                \end{enumerate}
            \item \ref{class_instance_list}のファイル名を、「\textless \ref{id_instance_word}で読み込んだ単語名\textgreater.csv」として保存する。
